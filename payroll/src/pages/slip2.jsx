...
function PayslipGenerator2() {
  ...
  // Function to extract employee data from PAYROLL1.xlsx format
  const extractEmployeeDataFromExcel = (workbook) => {
    ...
    try {
      ...
      // Employee rows based on PAYROLL1.xlsx structure
      // Rows: SB MTO MENRO section
      const employeeRows = [15, 16, 17, 21, 22, 23, 24, 25, 29, 30, 34, 35];
      const employees = [];
      
      employeeRows.forEach(row => {
        try {
          // Get cell values from PAYROLL1 format
          ...
          const cellH = firstSheet[`H${row}`];  // AMOUNT ACCRUED FOR THE PERIOD (actually the monthly rate/accrued)
          ...
          if (cellC && cellC.v) {
            ...
            // Calculate earnings - in PAYROLL1, the monthly rate is in column G (SALARY)
            // and sometimes it's half of the monthly salary (for half-month payroll)
            let monthlyRate = 0;
            let amountAccrued = 0;
            ...
            {payslips.length > 0 ? (
              ...
              <div className="col-span-1 sm:col-span-2 text-center py-12">
                ...
                <p className="mt-2 text-gray-400 max-w-md mx-auto">
                  {location.state?.fileData 
                    ? "The Excel file doesn't contain payroll data in the PAYROLL1 format."
                    : "Please upload a PAYROLL1 Excel file with payroll data to generate payslips."
                  }
                </p>
                {!location.state?.fileData && (
                  <button
...
export default PayslipGenerator2;