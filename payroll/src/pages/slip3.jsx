...
function PayslipGenerator3() {
  ...
  // Function to extract employee data from PAYROLL3.xlsx format
  const extractEmployeeDataFromExcel = (workbook) => {
    ...
    try {
      ...
      // Rows: MHO, Nurses, Midwives, etc. (RHU section) and MASO, LCRO, MSWDO sections
      const employeeRows = [15, 16, 17, 18, 19, 20, 21, 25, 26, 27, 28, 32, 33, 37, 38];
      const employees = [];
      
      employeeRows.forEach(row => {
        try {
          // Get cell values from PAYROLL3 format
          ...
          if (cellC && cellC.v) {
            ...
            // Calculate earnings - in PAYROLL3, the monthly rate is in column G (SALARY)
            let monthlyRate = 0;
            let amountAccrued = 0;
    ...
    <div className="fixed inset-0 w-full h-full bg-[#0a0a0f] overflow-auto">
      ...
      <div className="w-full h-full min-h-screen">
        <div className="w-full px-3 pt-20 mt-2">
          <h1 className="text-2xl font-bold mb-2 text-white">Liloan Payslip Generator (PAYROLL3 Format - RHU/MASO/MCR/HRMO)</h1>
            ...
            {payslips.length > 0 ? (
              ...
              <div className="col-span-1 sm:col-span-2 text-center py-12">
                ...
                <p className="mt-2 text-gray-400 max-w-md mx-auto">
                  {location.state?.fileData 
                    ? "The Excel file doesn't contain payroll data in the PAYROLL3 format."
                    : "Please upload a PAYROLL3 Excel file with payroll data to generate payslips."
                  }
                </p>
                {!location.state?.fileData && (
                  <button
...
export default PayslipGenerator3;